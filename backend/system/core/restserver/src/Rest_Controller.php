 ...
 * @version         4.0.0
 */
class Rest_Controller extends \Sleekwaredb_Controller
{
    /**
    ...
    {
        parent::__construct();
        $this->storedb = new Sleekwaredb_Model;
        ...
        if ($this->config->item('rest_enable_logging') === true) {
            ...
            $this->_start_rtime = round(microtime(true) * 1000);;
        }
        ...
        elseif (property_exists($this, 'storedb')) {
            $this->rest->db = $this->storedb;
        }
    ...
    public function __destruct()
    {
        ...
        if ($this->config->item('rest_enable_logging') === true) {
            ...
            $this->_end_rtime = round(microtime(true) * 1000);
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            isset($row['email']) && $this->rest->email = $row['email'];
    ...
    protected function _log_request($authorized = false)
    {
        ...
        $this->_insert_id = $is_inserted['_id'];
    ...
    protected function _check_limit($controller_method)
    {
        ...
        elseif ($result['hour_started'] < (time() - $time_limit)) {
            ...
            $limit_data['hour_started'] = time();
            $limit_data['count'] = 1;

            $store_limits->update($limit_data);
        }
        ...
        // They have called within the hour, so lets update
            ...
            $limit_data['count'] = $result['count'] + 1;
    ...
    protected function getAuth()
    {
        if ($this->getBearerToken()) {
            $authorized = $this->getBearerToken();
            $splitter = explode('.', $authorized);
            $pharse = end($splitter);
            array_pop($splitter);
            $token = implode('.', $splitter);
            return [
                'pharse' => $pharse,
                'token' => $token
            ];
            ...
            $this->response([
                'status' => false,
                'type' => 'error',
                'msg' => 'Fatal Error! Undefined Authorization Token, please generate first.'
            ]);
        }
    }
}
